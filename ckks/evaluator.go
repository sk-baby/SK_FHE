...
type Evaluator interface {
	...
	mulSRelin(op0 Operand, op1 []Operand, relin bool, ctOut *Ciphertext)
	MulRelinNew(op0, op1 Operand) (ctOut *Ciphertext)
	MulSRelinNew(op0 Operand, op1 []Operand) (ctOut *Ciphertext)
...
// MulSRelinNew Add By SK
// one Ciphertext mul many Plaintext ,output one Ciphertext
// This Alg is the mul of ring
func (eval *evaluator) MulSRelinNew(op0 Operand, op1 []Operand) (ctOut *Ciphertext) {
	//TODO implement me
	ctOut = NewCiphertext(eval.params, 1, utils.MinInt(op0.Level(), op1[0].Level()), 0)
	eval.mulSRelin(op0, op1, true, ctOut)
	panic("implement me")
}
...
// one Ciphertext mul many Plaintext ,output one Ciphertext
// This Alg is the mul of ring
func (eval *evaluator) mulSRelin(op0 Operand, op1 []Operand, relin bool, ctOut *Ciphertext) {

	//eval.checkBinary(op0, op1, ctOut, utils.MaxInt(op0.Degree(), op1.Degree()))
	P := ctOut.CopyNew()
	level := utils.MinInt(op0.Level(), op1[0].Level())
	...
	P.Scale = op0.ScalingFactor() * op1[0].ScalingFactor()
	ringQ := eval.ringQ

	var c00, c01, c0, c1, c2 *ring.Poly

	// Case Ciphertext (x) Ciphertext
	if op0.Degree()+op1[0].Degree() == 2 {
		...
		if relin == false {
			if P.Degree() < 2 {
				P.El().Resize(eval.params.Parameters, 2)
			}
			c2 = P.Value[2]
		} else {
			c2 = eval.poolQMul[2]
		...
		for i := 0; i < len(op1); i++ {
			if op1[i].El() == P.El() {
				tmp0, tmp1 = op1[i].El(), op0.El()
			} else {
				tmp0, tmp1 = op0.El(), op1[i].El()
			}

			ringQ.MFormLvl(level, tmp0.Value[0], c00)
			ringQ.MFormLvl(level, tmp0.Value[1], c01)

			if op0 == op1[i] { // squaring case
			...
			if relin {
				...
				ringQ.AddLvl(level, c0, eval.PoolQ[1], P.Value[0])
				ringQ.AddLvl(level, c1, eval.PoolQ[2], P.Value[1])
			}
		...
		for i := 0; i < len(op1); i++ {
			...
			ringQ.MFormLvl(level, tmp0.Value[0], c00)
			ringQ.MulCoeffsMontgomeryLvl(level, c00, tmp1.Value[0], P.Value[0])
			ringQ.MulCoeffsMontgomeryLvl(level, c00, tmp1.Value[1], P.Value[1])
			P.Value[0].ZeroOne(i)
			P.Value[1].ZeroOne(i)
			eval.Add(ctOut, P, ctOut)
		}
	}
}
